...
app.post('/analyze', async (req, res) => {
  const { code, language, questionData } = req.body;

  try {
    let promptText = "";
    
    if (language === "hint-generation" && questionData) {
      // Special handling for hint generation
      promptText = `Generate 3-4 helpful coding hints for solving this programming problem. Make the hints progressive (easy to advanced) and specific to help students think through the solution without giving away the complete answer.
...
Sample Test Cases: ${JSON.stringify(questionData.testCases, null, 2)}

Please provide hints that:
1. Help understand the problem approach
2. Suggest relevant data structures or algorithms
3. Point out important edge cases or constraints to consider
4. Guide towards an efficient solution

Format each hint on a new line and make them encouraging and educational.`;
    } else {
      // Regular code analysis
      promptText = `Analyze the following ${language} code. Give feedback on:
- Code quality
- Readability
- Optimization
- Maintainability
